...
pacman::p_load(officedown, officer) # processes BLOCK_LANDSCAPE markers in Word output
...
kable(
  ...
  caption = "(\\#tab:like-comp-tot) Likelihood components by category and model."
)
...
pander::pander(
  round(PlotTab, 2), 
  split.cells = 10, 
  caption = "(\\#tab:obscatch) Comparison of observed retained catches, discards, and bycatch between base and hybrid cases (Units: 1,000 tonnes)."
)
...
ref_table <- names(model_defs) |>
  ...
  dplyr::mutate(Model = as.character(to_short(Model))) |>
...
kable(
  ...
  caption = "(\\#tab:stepchange) Management quantities derived from maximum likelihood estimates by model using Tier 3 reference points. Reported natural mortality is for mature males, average recruitment is for males, and status and MMB were estimates for February 15 of the completed crab year."
)
```
<!---BLOCK_LANDSCAPE_STOP--->
\elandscape
...
```{r predtotnum, echo=FALSE, warning=FALSE, message=F}
...
<!---BLOCK_LANDSCAPE_START--->
\blandscape
```{r mmbfits, echo=FALSE, warning=FALSE, message=F, fig.width=8.5, fig.height=8, fig.cap="Model fits to the observed mature biomass at survey."}
...
```{r obs-101-vs-pred, echo=FALSE, warning=FALSE, message=F, out.width="100%", fig.cap="Estimated biomass of male crab >101mm carapace width from the survey (black line and dots with gray 95th CI) and from each model in the assessment (colored lines)."}
...
```{r select-prior, echo=FALSE, warning=FALSE, message=F, fig.height=10, fig.width=7, fig.cap="Estimated survey selectivity (lines) with normal priors derived from BSFRF selectivity experiment data. Points are the mean of the prior at a given size; intervals are 95th quantiles based on input CVs."}
...
```{r predfmort, echo=FALSE, warning=FALSE, message=F, fig.height=10, fig.width=8, fig.cap="Estimated fishing mortalities for the directed and non-directed fisheries."}
...
```{r predmat, echo=FALSE, warning=FALSE, message=F, fig.height=10, fig.width=8, fig.cap="Estimated (black line) or specified (colored lines) probability(s) of maturing for male crab."}
...
```{r recruits, echo=FALSE, warning=FALSE, message=F, fig.height=10, fig.width=8, fig.cap="Estimated recruitment by sex (bottom) and proportions recruiting to length bin (top) by model."}
...
```{r nat-m, echo=FALSE, warning=FALSE, message=F, fig.height=10, fig.width=8, fig.cap="Estimated natural mortality by sex and maturity state. Natural mortality in all years previous to 2018 and after 2019 are equal to the estimated M in 2017."}
...
```{r predmmb, echo=FALSE, warning=FALSE, message=F, fig.height=8, fig.width=8, fig.cap="Model predicted mature male biomass at mating time in 1,000 tonnes. Dashed horizontal lines are half of the respective BMSY proxies based on B35% of a given management currency (i.e. MSST)."}
#=========================================================
...
dfr = wtsGMACS::extractRep1Results(reslst,"Summary") |>
  ...
  dplyr::mutate(across(c(year, MMB), as.numeric)) |>
  dplyr::mutate(case = to_short(case))

#==add reference points back in
ggplot(dfr)+
  geom_line(aes(x=year,y=MMB,col=case))+
