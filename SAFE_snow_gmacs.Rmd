...
if(do_size_comps==1){
  ...
  for (i in seq_along(unique(unique_comps$comp_id))) { # Loop around comp data types
    ...
    ggsave(paste("plots/size_comp_resid_", curr_id, "_", compname,".png"), p_res, height = 10, width = 8, dpi = 300)
  }
}
...
# 1. Get a list of all comp plots in the directory
img_folder <- "plots"
img_files <- list.files(path = img_folder, pattern = "size_comp_plot", full.names = TRUE)

# 2. Loop through each file and include it on a new page
for (f in img_files) {
  # Insert the image using Markdown syntax
  cat(paste0("![Model fits (lines) to size composition data (grey bars)](", f, ")\n\n"))
  
  # Insert a page break (different syntax for PDF vs HTML)
  # For PDF output:
  cat("\\newpage \n")
}
