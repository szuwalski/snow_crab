...
PlotTab<- data.frame(Model=names(model_defs),
                     MMB=rep(0,length(model_defs)),
                     B35=rep(0,length(model_defs)),
                     F35=rep(0,length(model_defs)),
                     FOFL=rep(0,length(model_defs)),
                     OFL=rep(0,length(model_defs)),
                     M=rep(0,length(model_defs)),
                     avg_rec=rep(0,length(model_defs)),
                     Status=rep(0,length(model_defs)))

take_row<-c(1,2,3,4,8,12,13)
keep_out<-matrix(nrow=length(model_defs),ncol=length(take_row))
...
rownames(man_tab) = names(model_defs)
...
p = ggplot(dfrIndxFit,aes(x=year,colour=model)) +
  geom_point(aes(y=obs),position = position_dodge(width = 1)) +
...
tmp3<-paste("reslst$outsLst$'","25.1 gmacs (up + mature)","'$mature_probability",sep='')
tmp<-eval(parse(text=tmp2))
tm<-eval(parse(text=tmp3))
...
plotdat<-melt(tm,id.vars=c("Sex","Year"))
colnames(plotdat)[3:4]<-c("width","prob")
plotdat[,3]<-as.numeric(as.character(plotdat[,3]))
plotdat[,4]<-as.numeric(plotdat[,4])
q <- ggplot() +
