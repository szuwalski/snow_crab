...
```{r idx-imm, fig.width=8, fig.height=4.4, out.width="100%"}
...
ggplot(idx_imm, aes(x = year, y = obs, group = Scenario)) + # Added group = Scenario here
    geom_errorbar(
        aes(ymin = lba, ymax = uba, colour = Scenario), 
        width = 0, 
        linewidth = 1.5,
    ...
    geom_point(
        colour = "black", 
        size = 1.3, 
        position = position_dodge(width = 1) # Now it knows what to dodge by
    ) +
    ...
    labs(
        ...
        colour = NULL,
        caption = "Immature index introduces a new data source..."
    ) +
    theme_slide(8)
```
...
- **`sdmTMB` spatiotemporal model** fit to chela-based maturity classifications.
...
- Replaces the legacy "fit a curve to chela ratios per year/size" approach.
- Has a **much weaker temporal trend** in $\Pr$(terminal molt) at intermediate sizes than the legacy workflow.
...
idx_mat_nm <- wtsGMACS::extractRep1Results(reslst, "Index_fit_summary") |>
  ...
  dplyr::mutate(
    across(c(year, obs, actual_CV), as.numeric),
    Scenario = dplyr::recode(case,
      "25.1 gmacs (update + compfix + plus group + imm_surv)"           = "base (old maturity)",
      "25.1 gmacs (update + compfix + plus group + new_mat + imm_surv)" = "+ new maturity"
    ),
  ...
  dplyr::filter(Sex == "Male") |>
  dplyr::distinct(Scenario, year, Maturity, Sex, obs, lba, uba)

ggplot(idx_mat_nm, aes(x = year, y = obs, group = Scenario)) + # Added group here
    geom_errorbar(
        aes(ymin = lba, ymax = uba, colour = Scenario), # Color mapped only to bars
        width = 0, 
        linewidth = 1,
        position = position_dodge(width = 1), 
        alpha = 1
    ...
    geom_point(
        colour = "black", # Points forced to black
        size = 1.3, 
        position = position_dodge(width = 1) # Now inherits the group and dodges correctly
    ) +
    facet_wrap(~Maturity, scales = "free_y", ncol = 2) +
    scale_y_continuous(limits = c(0, NA), expand = expansion(mult = c(0, 0.08))) +
    scale_colour_manual(values = c("#af8dc3", "#7fbf7b")) +
    labs(
        x = "Year", 
...
ggplot(surv_nm, aes(x = size, y = aggObs, colour = Scenario)) +
  ...
  labs(x = "Carapace width (mm)", y = "Observed proportion",
       colour = NULL,
       caption = "NMFS 1989-present mature-male size composition.") +
  theme_slide(8)
```
...
- Hybrid data are **additive** to existing snow-crab time series
- CV on directed discards held at 0.7. Observer non-directed bycatch unchanged.
- Retained hybrids cannot be separated from retained snow crab (hybrids already retained under snow IFQ; cannot be separated).
...
- Visibly **higher predicted survey biomass** in some years; trend through time is unchanged.
- Hybrid identification quality **decreases further back** in the time series (consistency between ADF&G and NMFS criteria is currently an issue).
...
ggplot(idx_mat_hyb, aes(x = year, y = obs, group = Scenario)) + # Added group = Scenario here
    # 1. Error bars get the Scenario colors
    geom_errorbar(
        aes(ymin = lba, ymax = uba, colour = Scenario), 
        width = 0, 
        linewidth = 1,
    ...
    # 2. Points are fixed to black
    geom_point(
        colour = "black", 
        size = 1.3, 
        position = position_dodge(width = 1) # Now inherits the group and dodges
    ) +
    facet_wrap(~Sex, scales = "free_y", ncol = 2) +
    scale_y_continuous(
        limits = c(0, NA), 
    ...
    # 3. Applying the same color palette as the first plot
    scale_colour_manual(values = c("#af8dc3", "#7fbf7b")) +
    labs(
        x = "Year", 
        y = "NMFS survey biomass (1,000 t)",
...
- **Only `Model 25.1d`** had a max |gradient| < 0.001.
...
- Mature male M is > 0.6 for these models
...
- Resolving the underlying source of multiple optima will require **further investigation** before September:
  ...
  - Should we include the immature index?
...
## Model fits -- mature biomass survey index

```{r mmbfits, fig.width=11, fig.height=4.7, out.width="100%"}
# Model fits to the observed mature biomass survey index, all candidate models
# except the rolled-forward `25.1 gmacs` reference. Panels facet by sex and by
# maturity workflow so the new-maturity reclassification effect is visible.
mmbfits <- wtsGMACS::extractRep1Results(reslst, "Index_fit_summary") |>
  dplyr::filter(case != "25.1 gmacs", maturity == "mature") |>
  dplyr::mutate(across(c(year, obs, actual_CV, prd), as.numeric)) |>
  dplyr::rename(predicted = prd, model = case) |>
  dplyr::mutate(
...
obs_mmb <- mmbfits |>
  dplyr::distinct(year, Sex, maturity_status, hybrid_status, obs, lba, uba)

ggplot() +
  geom_line(data = mmbfits,
            aes(x = year, y = predicted, colour = model),
            lwd = 0.7, alpha = 0.85) +
  geom_errorbar(data = mmbfits,
                aes(x = year, ymin = lba, ymax = uba),
                width = 0, alpha = 0.5, colour = "black") +
  geom_point(data = mmbfits,
             aes(x = year, y = obs),
             size = 0.9, colour = "black") +
  scale_y_continuous(limits = c(0, NA), expand = expansion(mult = c(0, 0.08))) +
  facet_grid(Sex ~ maturity_status + hybrid_status, scales = "free_y") +
  labs(x = "Year", y = "NMFS mature survey biomass (1,000 t)",
       colour = "Model",
       caption = "Points/bars: observations + 95% lognormal CI. Lines: model fits. Reference model 25 excluded.") +
  theme_slide(8) +
  theme(legend.text = element_text(size = 7))
```

## Model fits -- immature biomass survey index

```{r immfits, fig.width=11, fig.height=4.7, out.width="100%"}
# Model fits to the observed immature biomass survey index. Only models with
# the immature index contribute predictions; observations differ between the
# old- and new-maturity workflows.
immfits <- wtsGMACS::extractRep1Results(reslst, "Index_fit_summary") |>
...
obs_imm <- immfits |>
  dplyr::distinct(year, Sex, maturity_status, obs, lba, uba)

ggplot() +
  geom_line(data = immfits,
            aes(x = year, y = predicted, colour = model),
            lwd = 0.7, alpha = 0.85) +
  geom_errorbar(data = obs_imm,
                aes(x = year, ymin = lba, ymax = uba),
                width = 0, alpha = 0.5, colour = "black") +
  geom_point(data = obs_imm,
             aes(x = year, y = obs),
             size = 0.9, colour = "black") +
  scale_y_continuous(limits = c(0, NA), expand = expansion(mult = c(0, 0.08))) +
  facet_grid(Sex ~ maturity_status, scales = "free_y") +
  labs(x = "Year", y = "NMFS immature survey biomass (1,000 t)",
       colour = "Model",
       caption = "Only imm_surv models contribute predictions. New maturity workflow shifts observed immature biomass between mature/immature classes.") +
  theme_slide(8) +
  theme(legend.text = element_text(size = 7))
```
...
### Retained-male comp fits, across `compfix` models

![](plots/size_comp_plot_43_Pot_fishery_compfix_male_retained_undetermined_Old maturity.png){height="5.9in"}
...
![](plots/size_comp_plot_38_Pot_Fishery_male_total_undetermined_Old maturity.png){height="5.9in"}

![](plots/size_comp_plot_35_Pot_Fishery_male_total_undetermined_New maturity.png){height="5.9in"}



### NMFS-immature female comp fits, old vs. new-maturity


![](plots/size_comp_plot_21_NMFS_Trawl_1989_female_discarded_immature_Old maturity.png){height="5.9in"}


![](plots/size_comp_plot_17_NMFS_Trawl_1989_female_discarded_immature_New maturity.png){height="5.9in"}
